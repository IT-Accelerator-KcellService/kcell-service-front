...
// Детекция смены аккаунта теперь реализована надёжно через Zustand subscribe
// в компоненте BridgeInit.tsx (подписка на useAuthStore).
// localStorage.setItem override НЕ используется — он ненадёжен:
// Zustand persist может обойти override, clearAuth() пишет {token:null} а не removeItem,
// и periodic check останавливается через 30 секунд.
